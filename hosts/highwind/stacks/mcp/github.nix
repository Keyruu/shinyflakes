...
in
{
  ...
  systemd.services.mcp-github = {
    ...
    serviceConfig = {
      ExecStart = "${lib.getExe mcp-proxy} --port 30000 --host 0.0.0.0 --pass-environment -- ${lib.getExe pkgs.podman} run -i --rm -e GITHUB_PERSONAL_ACCESS_TOKEN ghcr.io/github/github-mcp-server";
      User = "root";
      Group = "root";
      Restart = "always";
