...
let
  ...
  waybar-dunst-monitor = pkgs.writeShellApplication {
    name = "waybar-dunst-monitor";
    runtimeInputs = with pkgs; [
      dunst
      dbus
      jq
    ...
    text = # bash
      ''
        ...
        exec dbus-monitor --profile "path='/org/freedesktop/Notifications',interface='org.freedesktop.DBus.Properties',member='PropertiesChanged'" | \
        while read -r _; do
