{ ... }:
{
  den.aspects.server.monitoring-infra = {
    nixos =
      ...
      config = {
        networking.firewall.interfaces."${config.services.monitoring.metrics.interface}".allowedTCPPorts = [
        ...
        services = {
          ...
          prometheus.exporters = lib.mkIf config.services.monitoring.metrics.enable {
            node = {
              enable = true;
              enabledCollectors = [
                "systemd"
          ...
          fluent-bit = lib.mkIf config.services.monitoring.logs.enable {
            enable = true;
            settings = {
              service = {
                flush = 5;
              ...
              pipeline = {
                ...
                filters = [
                  {
                    ...
                    Add = [
                      ...
                      "instance ${config.services.monitoring.logs.instance}"
                    ];
                  }
                  {
                ...
                outputs = [
                  (
                    let
                      parts = builtins.match "http://([^:]+):([0-9]+)" config.services.monitoring.logs.lokiAddress;
                    in
                    {
                      name = "loki";
                      match = "*";
